...
void esp8266ArtNetRDM::init(IPAddress ip, IPAddress subnet, bool dhcp, char* shortname, char* longname, uint16_t oem, uint16_t esta, uint8_t* mac) {
  ...
  strncpy(_art->shortName, shortname, ARTNET_SHORT_NAME_LENGTH);
  strncpy(_art->longName, longname, ARTNET_LONG_NAME_LENGTH);
...
void esp8266ArtNetRDM::handler() {
  ...
  // Ehm no? The specs say is should only be sent as a response to ArtPoll and as a single broadcast at power on
  //_artPoll();
...
void esp8266ArtNetRDM::setShortName(char* name) {
  if (_art == 0)
    return;
  strncpy(_art->shortName, name, ARTNET_SHORT_NAME_LENGTH);
}
...
void esp8266ArtNetRDM::setLongName(char* name) {
  if (_art == 0)
    return;
  strncpy(_art->longName, name, ARTNET_LONG_NAME_LENGTH);
}
